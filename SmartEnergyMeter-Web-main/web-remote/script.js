...
const meter = { voltage: 0, current: 0, power: 0, energy: 0, frequency: 0, pf: 0, va: 0, var: 0, limit: 0, wifi: false, sensor: false, trip: false, temperature: 0, humidity: 0, dht: false, chipTemperature: null };
...
function clearMetricsDisplay() {
  ["metric-watt", "metric-kwh", "metric-voltage", "metric-current", "metric-freq", "metric-pf", "metric-va", "metric-var", "metric-temp", "metric-hum", "metric-chip"].forEach((id) => setText(id, "-"));
  ["watt-percent-label", "metric-limit", "metric-remaining", "watt-percent-small", "pf-label", "chip-label"].forEach((id) => setText(id, "-"));
  ["gauge-watt", "gauge-kwh", "gauge-voltage", "gauge-current", "gauge-freq", "gauge-pf", "gauge-va", "gauge-var", "gauge-temp", "gauge-hum", "gauge-chip"].forEach((id) => setGauge(id, id === "gauge-watt" ? CIRC_LARGE : CIRC_SMALL, 0));
...
function updateDashboard() {
  ...
  // Absent on firmware older than the chip-temperature update, so "-"
  ...
  setText("chip-label", !hasChip ? "-" : chip >= 85 ? "Terlalu panas" : chip >= 70 ? "Hangat" : "Normal");
